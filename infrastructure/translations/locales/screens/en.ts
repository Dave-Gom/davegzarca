import type { ScreensTranslations } from "@/infrastructure/types/translations/screens";

const screens: ScreensTranslations = {
  home: {
    heroSection: {
      basedIn: "Based in Areguá, Paraguay",
      heroName: "David Gómez.",
      heroTagline: "Building Mobile Apps Used by Millions.",
      heroDescription:
        "Senior Fullstack Mobile & Web Developer specializing in React Native and Native iOS architectures, backed by Node.js backends for the financial and healthcare sectors.",
      viewWork: "View My Work",
      getInTouch: "Get in Touch",
      portraitAlt: "Photo of David Gómez",
    },
    aboutSection: {
      philosophyLabel: "The Philosophy",
      philosophyTitle:
        "Code is the starting point; scalability and team leadership are the differentiators.",
      philosophyP1Start:
        "With over 6 years of experience, I contribute to and lead development teams throughout the full lifecycle of high-impact mobile products. My approach centers on ",
      philosophyP1Bold1: "efficient, scalable code",
      philosophyP1Mid: " and ",
      philosophyP1Bold2: "reliable delivery",
      philosophyP1End: ".",
      philosophyP2:
        "I serve as the bridge between technical execution and business goals from architecture decisions and state management to API design and team coordination always focused on delivering solutions that stand up at scale.",
      yearsCount: "6+",
      yearsLabel: "Years of Experience",
      appsCount: "5",
      appsLabel: "Published Apps",
    },
    projectsSection: {
      worksLabel: "Selected Works",
      worksTitle: "Impact-Driven Engineering",
      project1Alt: "Ploutos Personal Finance App",
      project1Title: "Ploutos",
      project1Desc:
        "Sole developer of a cross-platform personal finance app built with React Native and Expo SDK 54, running natively on iOS and Android. Offline-first architecture using encrypted SQLite, multi-currency support with live exchange rates, hierarchical categories, statistics with charts, and Mobills data import. Integrated with Firebase Analytics, Crashlytics, and Remote Config.",
      project1Impact: "React Native",
      project1ImpactLabel: "End-to-End Solo Build",
      project1Role: "Sole Developer",
      project2Alt: "Billetera Mango Fintech App",
      project2Title: "Billetera Mango",
      project2Desc:
        "React Native developer for one of Paraguay's most used fintech apps. Developing and maintaining high-quality features for an electronic wallet with over 2 million active users, with a strong focus on performance and scalability.",
      project2Impact: "2M+ Active Users",
      project2ImpactLabel: "Scale & Impact",
      project2Role: "React Native Developer Sr.",
    },
    experienceSection: {
      historyLabel: "History",
      historyTitle: "Career Milestones",
      role1Period: "2024 — Pres.",
      role1Title: "React Native Developer Sr. — Billetera Mango",
      role1Desc:
        "Developing and maintaining new features for Paraguay's leading electronic wallet. Collaborating within a team focused on high-quality UX, performance optimization, and scalability.",
      role1Item1:
        "Contributing to a platform serving over 2 million active users.",
      role1Item2:
        "Focused on performance, code quality, and scalable architecture with React Native, TypeScript, Redux, Sentry, OneSignal, and Rive.",
      role2Period: "2022 — 2024",
      role2Title: "React Native Developer — Banco Continental SAECA",
      role2Desc:
        "Worked on two major banking apps: Conti Móvil (retail) and Continental Banca Empresas (corporate). Shipped features and resolved issues ensuring a seamless banking experience for hundreds of thousands of users.",
      role2Item1:
        "Contributed to Conti Móvil, a mobile banking app with 450,000+ users.",
      role2Item2:
        "Took part in continuous product improvement under Agile/Scrum methodologies.",
      role3Period: "2022",
      role3Title: "Fullstack Developer — Arco SA",
      role3Desc:
        "Worked as a Fullstack Developer managing and enhancing the core web system of a radiology clinic, building modules for patient management, billing, and role-based access control.",
      role4Period: "2019 — 2021",
      role4Title: "Junior Developer — Sercicronos",
      role4Desc:
        "Started my career as a Junior Developer at Sercicronos, a marathon and running event management platform.",
      role5Period: "2024 — Pres.",
      role5Title: "Mobile & Frontend Developer — PadelSys",
      role5Desc:
        "Mobile Frontend Developer for PadelSys, a padel tournament management platform used across Paraguay. Working in close collaboration with the product stakeholder, taking on the Product Owner role, for the development of the mobile app: tournament creation, player registration, bracket generation, and analytics.",
    },
    skillsSection: {
      skillsLabel: "Core Competencies",
      skillsTitle: "The Technical Stack",
      mobileTitle: "Mobile Development",
      webTitle: "Web Development",
      backendTitle: "Back-end & Systems",
      devopsTitle: "DevOps & Tools",
    },
    contactSection: {
      ctaLabel: "Open to New Opportunities",
      ctaTitle: "Let's build something that scales.",
      ctaLinkedIn: "LinkedIn Profile",
    },
  },
  about: {
    heroSection: {
      subtitle: "Senior Fullstack Mobile & Web Developer",
      title: "About Me",
      p1: "I'm a Fullstack Mobile & Web Developer with over 6 years of experience building high-performance applications used by millions of people in Paraguay. I specialize in React Native, TypeScript, and Node.js, with a proven track record in the financial sector.",
      p2: "My focus is on delivering clean, scalable, and maintainable code while actively contributing to team growth. I thrive in Scrum environments, leading projects from architecture through delivery — both as an individual contributor and as a team lead.",
      portraitAlt: "Professional portrait of David Gómez",
    },
    philosophySection: {
      philosophyTitle: "My Philosophy",
      archTitle: "Architectural Integrity",
      archDesc:
        "I believe in building structures that let teams move fast without piling up unsustainable technical debt. Every module should be as easy to replace as it is to build.",
      precisionTitle: "Strategic Precision",
      precisionDesc:
        "Every technical choice is a business decision. I evaluate library longevity, team familiarity, and performance tradeoffs before committing to an architectural direction.",
      craftTitle: "Code as Craft",
      craftDesc:
        "Quality is non-negotiable. I champion clean, expressive codebases, rigorous code reviews, and a culture of continuous improvement.",
      humanTitle: "Human Centricity",
      humanDesc:
        "Great apps are built by great teams. I focus on clear communication, well-documented APIs, and reducing cognitive load for everyone involved in the product.",
    },
    beyondCodeSection: {
      title: "Beyond the Code",
      subtitle: "Leadership & Impact",
      mentoringCard: {
        label: "Mentoring",
        title: "Leading Distributed Teams",
        description:
          "As project lead for Farmacia Canaria, I directed a distributed team of 3 developers while serving as the sole backend developer translating client requirements into functional, role-based modules for a telemedicine platform.",
        workshops: "Team Coordination",
        coaching: "Technical Ownership",
      },
      openSourceCard: {
        label: "Freelance",
        title: "End-to-End Solo Projects",
        description:
          "Mobile & Frontend Developer for PadelSys, a padel tournament platform used across Paraguay, working closely with the product stakeholder. I also built the Realtec landing page as an independent project, negotiating scope directly with the client.",
        stars: "1K+ Users Served",
      },
      speakingCard: {
        imageAlt: "David Gómez working on code",
        title: "Technical Leadership",
        description:
          "At Farmacia Canaria I led the project's technical decision-making: defined the backend architecture, established coding standards, and guided the team through complex integration challenges with existing healthcare systems.",
      },
      appearancesCard: {
        title: "Recent Projects",
        appearance1: "PadelSys",
        appearance1Detail: "Padel Tournaments",
        appearance2: "Billetera Mango",
        appearance2Detail: "Fintech · 2M+ Users",
        appearance3: "Conti Móvil",
        appearance3Detail: "Banking · 450K+ Users",
      },
    },
  },
  experience: {
    header: {
      subtitle: "Professional History",
      title: "Experience",
      description:
        "Over 6 years building high-impact mobile and web products across finance, healthcare, and sports. From banking apps with 450K users to a fintech wallet with over 2 million.",
    },
    projects: {
      worksLabel: "Published Projects",
      worksTitle: "Impact-Driven Engineering",
      project1Alt: "Billetera Mango Fintech App",
      project1Title: "Billetera Mango",
      project1Desc:
        "React Native developer for one of Paraguay's most used fintech apps. Developing and maintaining high-quality features for an electronic wallet with over 2 million active users, with a strong focus on performance and scalability.",
      project1Impact: "2M+ Active Users",
      project1ImpactLabel: "Scale & Impact",
      project1Role: "React Native Developer Sr.",
      project2Alt: "Conti Móvil Banking App",
      project2Title: "Conti Móvil: Banco Continental",
      project2Desc:
        "Contributed to the development of Conti Móvil, Paraguay's leading mobile banking app, and Continental Banca Empresas for corporate clients. Shipped new features and resolved front-end issues under Scrum methodology.",
      project2Impact: "500K+ Users",
      project2ImpactLabel: "National Banking Scale",
      project2Role: "React Native Developer",
      project3Alt: "Ploutos Personal Finance App",
      project3Title: "Ploutos",
      project3Desc:
        "Sole developer of a cross-platform personal finance app built with React Native and Expo SDK 54, running natively on iOS and Android. Offline-first architecture using encrypted SQLite, multi-currency support with live exchange rates, hierarchical categories, statistics with charts, and Mobills data import. Integrated with Firebase Analytics, Crashlytics, and Remote Config.",
      project3Impact: "React Native",
      project3ImpactLabel: "End-to-End Solo Build",
      project3Role: "Sole Developer",
      project4Alt: "Continental Banca Empresas Banking App",
      project4Title: "Continental Banca Empresas",
      project4Desc:
        "Contributed to the development of Continental Banca Empresas, Banco Continental's corporate banking app, as part of the bank's mobile team. Shipped new features and resolved issues for corporate account management, transfers, salary payments, service payments, and operation authorizations under Scrum methodology.",
      project4Impact: "Banco Continental",
      project4ImpactLabel: "National Corporate Banking",
      project4Role: "React Native Developer",
      project5Alt: "PadelSys Padel Tournament Platform",
      project5Title: "PadelSys",
      project5Desc:
        "Mobile Frontend Developer for PadelSys, a comprehensive padel tournament management and club administration platform used across Paraguay. Working in close collaboration with the product stakeholder, taking on the Product Owner role, for the development of the mobile app: tournament creation, player registration, automatic bracket generation, circuit rankings, online reservations, and analytics.",
      project5Impact: "1K+ Users",
      project5ImpactLabel: "National Padel Platform",
      project5Role: "Mobile & Frontend Developer · Freelance",
    },
    timelineSectionLabel: "Career History",
    timelineSectionTitle: "Timeline",
    timeline: {
      current: "Current",
      years3: "2 Years",
      years2: "4 Months",
      period1: "2024 — PRESENT",
      company1: "Billetera Mango",
      role1: "React Native Developer Sr.",
      role1Item1:
        "Developing and maintaining new features for an electronic wallet with over 2 million active users.",
      role1Item2:
        "Collaborating within a Scrum team to deliver high-quality UX with a focus on performance and scalability.",
      role1Item3:
        "Working with React Native, UIKit, Kotlin, TypeScript, Swift, Redux, OneSignal, Sentry, and Rive.",
      period2: "2022 — 2024",
      company2: "Banco Continental SAECA",
      role2: "React Native Developer",
      role2Item1:
        "Contributed to Conti Móvil, Paraguay's mobile banking app with 450,000+ users.",
      role2Item2:
        "Developed features for Continental Banca Empresas, the bank's corporate client app.",
      role2Item3:
        "Shipped front-end improvements and bug fixes under Agile/Scrum methodology.",
      period3: "2022",
      company3: "Arco SA",
      role3: "Full Stack Developer",
      role3Item1:
        "Managed and enhanced the core web system of a radiology clinic.",
      role3Item2:
        "Built modules for patient management, billing, and role-based access control.",
      role3Item3: "Tech stack: Laravel, Vue.js, Express.js, HTML5, MySQL, PHP.",
      period4: "2019 — 2021",
      company4: "Sercicronos",
      years4: "2 Years",
      role4: "Junior Developer",
      role4Item1:
        "Developed and maintained a marathon and running event management platform.",
      role4Item2:
        "Built features for event registration, timing systems, and results publishing.",
      role4Item3: "Tech stack: HTML5, CSS3, JavaScript, PHP, MySQL.",
      period6: "2026 — PRESENT",
      company6: "Ploutos",
      years6: "Personal Project",
      role6: "Sole Developer",
      role6Item1:
        "Designed and built a cross-platform personal finance app with React Native and Expo SDK 54, running natively on iOS and Android.",
      role6Item2:
        "Offline-first architecture with encrypted SQLite, multi-currency support with live exchange rates, hierarchical categories, statistics with charts.",
      role6Item3:
        "Integrated with Firebase Analytics, Crashlytics, and Remote Config. Mobills data import support.",
      period5: "2024 — PRESENT",
      company5: "PadelSys",
      years5: "Freelance",
      role5: "Mobile & Frontend Developer",
      role5Item1:
        "Mobile Frontend Developer for a padel tournament management platform used across Paraguay, taking on the Product Owner role in close collaboration with the product stakeholder.",
      role5Item2:
        "Developed tournament creation, player registration, bracket generation, and analytics features.",
      role5Item3:
        "Tech stack: PHP, JavaScript, jQuery, Bootstrap, Chart.js, MySQL.",
    },
    cta: {
      ctaTitle: "Ready to build something interesting?",
      ctaDesc:
        "Open to senior mobile development roles and freelance consulting opportunities.",
      ctaButton: "Get In Touch",
      ctaDownload: "Download CV",
    },
  },
  contact: {
    hero: {
      availability: "Open to new opportunities",
      title: "Available for",
      titleLine2: "New Opportunities",
      description:
        "Let's build something that scales. I specialize in React Native and Native iOS mobile development, fullstack Node.js architectures, and technical project leadership.",
      imageAlt: "Asunción, Paraguay - Eje corporativo",
    },
    sidebar: {
      contactCard: {
        formTitle: "Get in Touch",
        emailLabel: "Email Address",
        nameLabel: "Full Name",
        namePlaceholder: "John Doe",
        visionLabel: "Project Vision",
        visionPlaceholder: "Briefly describe your project or the role...",
        sendButton: "Send Message",
        socialLabel: "Find Me Online",
      },
      trustIndicator: {
        trustText:
          "Worked on apps used by over 2 million people across Paraguay.",
      },
    },
  },
};

export default screens;
